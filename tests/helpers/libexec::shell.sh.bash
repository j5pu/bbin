...
shell_container() {
  ...
  PATH="/${BATS_BASENAME}/tests/fixtures/shell.sh:${PATH}"
  local c=() env=(-e PATH="${PATH}" -e MANPATH="${MANPATH}" -e INFOPATH="${INFOPATH}")
  local helper=("/${BATS_BASENAME}/tests/fixtures/shell.sh/${BATS_ARRAY[3]}") shell=( "${BATS_ARRAY[4]-}" )
  if [ "${BATS_ARRAY[3]}" = '-c' ]; then
    c=( "${BATS_ARRAY[3]}" )
    helper=( "/${BATS_BASENAME}/tests/fixtures/shell.sh/${BATS_ARRAY[4]}${BATS_ARRAY[5]:+ ${BATS_ARRAY[5]}}" )
    shell=()
  fi
  command=( docker run --rm -v "${BATS_TOP}:/${BATS_BASENAME}" "${env[@]}" \
